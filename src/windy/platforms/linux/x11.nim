...
proc pollEvents(window: Window) =
  ...
  while display.XCheckIfEvent(ev.addr, checkEvent, cast[pointer](window)):
    ...
    of xeButtonPress, xeButtonRelease:
      ...
      of 4: pushScrollEvent(vec2(0, 1.0)) # scroll up
      of 5: pushScrollEvent(vec2(0, -1.0)) # scroll down
      of 6: pushScrollEvent(vec2(-1.0, 0)) # scroll left
      of 7: pushScrollEvent(vec2(1.0, 0)) # scroll right
      else: discard
